                ...
                if (args.length == 0) {
                    ...
                    if (referenced != null) {
                        if (args[0].equals("0")) {
                            ...
                            player.sendMessage(ChatColor.GOLD + " » " + ChatColor.YELLOW + referenced.getDisplayName() + ChatColor.YELLOW + "'s gamemode set to SURVIVAL");
                        } else if (args[0].equals("1")) {
                            referenced.setGameMode(GameMode.CREATIVE);
                            player.sendMessage(ChatColor.GOLD + " » " + ChatColor.YELLOW + referenced.getDisplayName() + ChatColor.YELLOW + "'s gamemode set to CREATIVE");
                        } else if (args[0].equals("2")) {
                            referenced.setGameMode(GameMode.ADVENTURE);
                            player.sendMessage(ChatColor.GOLD + " » " + ChatColor.YELLOW + referenced.getDisplayName() + ChatColor.YELLOW + "'s gamemode set to ADVENTURE");
                        } else if (args[0].equals("3")) {
                            referenced.setGameMode(GameMode.SPECTATOR);
                            player.sendMessage(ChatColor.GOLD + " » " + ChatColor.YELLOW + referenced.getDisplayName() + ChatColor.YELLOW + "'s gamemode set to SPECTATOR");
                        } else {
